...
// Use ML-DSA-87 sign_internal known answer tests (KATs) from NIST ACVP Server to ensure functional correctness and compatibility
// of this ML-DSA library implementation with ML-DSA standard @ https://doi.org/10.6028/NIST.FIPS.204.
TEST(ML_DSA, ML_DSA_87_Sign_Internal_ACVP_KnownAnswerTests)
{
  ...
  while (true) {
    ...
    if (!std::getline(file, mu_line).eof()) {
      ...
      std::array<uint8_t, ml_dsa_87::SigByteLen> computed_sig{};
      const auto sign_ok = ml_dsa_87::sign_internal(rnd, skey, mu, computed_sig);
      ASSERT_TRUE(sign_ok);
...
// Use ML-DSA-87 verify_internal known answer tests (KATs) from NIST ACVP Server to ensure functional correctness and compatibility
// of this ML-DSA library implementation with ML-DSA standard @ https://doi.org/10.6028/NIST.FIPS.204.
TEST(ML_DSA, ML_DSA_87_Verify_Internal_ACVP_KnownAnswerTests)
{
  ...
  while (true) {
    ...
    if (!std::getline(file, mu_line).eof()) {
      ...
      const auto is_valid = ml_dsa_87::verify_internal(pkey, mu, sig);
