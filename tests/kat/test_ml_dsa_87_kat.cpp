...
// Use ML-DSA-87 known answer tests to ensure functional correctness and compatibility of this ML-DSA library
// implementation with ML-DSA standard @ https://doi.org/10.6028/NIST.FIPS.204.
TEST(ML_DSA, ML_DSA_87_KnownAnswerTests)
{
  ...
  while (true) {
    ...
    if (!std::getline(file, seed_line).eof()) {
      ...
      const auto has_signed = ml_dsa_87::sign(rnd, computed_skey, msg, ctx, computed_sig);
      const auto is_valid = ml_dsa_87::verify(computed_pkey, msg, ctx, computed_sig);
...
TEST(ML_DSA, ML_DSA_87_Sign_ACVP_KnownAnswerTests)
{
  ...
  while (true) {
    ...
    if (!std::getline(file, msg_line).eof()) {
      ...
      std::array<uint8_t, ml_dsa_87::SigByteLen> computed_sig{};
      const auto sign_ok = ml_dsa_87::sign(rnd, skey, msg, ctx, computed_sig);
      ASSERT_TRUE(sign_ok);
...
// Use ML-DSA-87 verify known answer tests (KATs) from NIST ACVP Server to ensure functional correctness and compatibility
// of this ML-DSA library implementation with ML-DSA standard @ https://doi.org/10.6028/NIST.FIPS.204.
TEST(ML_DSA, ML_DSA_87_Verify_ACVP_KnownAnswerTests)
{
  const std::string kat_file = "./kats/ml_dsa_87_verify.acvp.kat";
  std::fstream file(kat_file);
  ...
  while (true) {
    ...
    if (!std::getline(file, msg_line).eof()) {
      ...
      const auto is_valid = ml_dsa_87::verify(pkey, msg, ctx, sig);
...
TEST(ML_DSA, ML_DSA_87_Sign_Internal_MuFalse_ACVP_KnownAnswerTests)
{
  ...
  while (true) {
    ...
    if (!std::getline(file, msg_line).eof()) {
      ...
      std::array<uint8_t, ml_dsa_87::SigByteLen> computed_sig{};
      const auto sign_ok = ml_dsa_87::sign_internal(rnd, skey, msg, computed_sig);
      ASSERT_TRUE(sign_ok);
...
TEST(ML_DSA, ML_DSA_87_Verify_Internal_MuFalse_ACVP_KnownAnswerTests)
{
  ...
  while (true) {
    ...
    if (!std::getline(file, msg_line).eof()) {
      ...
      const auto is_valid = ml_dsa_87::verify_internal(pkey, msg, sig);
...
// Use ML-DSA-87 HashML-DSA sign known answer tests from NIST ACVP Server (SHA3-family only).
TEST(ML_DSA, ML_DSA_87_HashSign_ACVP_KnownAnswerTests)
{
  using hash_algorithm = ml_dsa_87::hash_algorithm;

  const std::string kat_file = "./kats/ml_dsa_87_hash_sign.acvp.kat";
  std::fstream file(kat_file);
  ...
  while (true) {
    ...
    if (!std::getline(file, msg_line).eof()) {
      ...
      if (hash_alg == "SHA3-224") {
        sign_ok = ml_dsa_87::hash_sign<hash_algorithm::SHA3_224>(rnd, skey, msg, ctx, computed_sig);
      } else if (hash_alg == "SHA3-256") {
        sign_ok = ml_dsa_87::hash_sign<hash_algorithm::SHA3_256>(rnd, skey, msg, ctx, computed_sig);
      } else if (hash_alg == "SHA3-384") {
        sign_ok = ml_dsa_87::hash_sign<hash_algorithm::SHA3_384>(rnd, skey, msg, ctx, computed_sig);
      } else if (hash_alg == "SHA3-512") {
        sign_ok = ml_dsa_87::hash_sign<hash_algorithm::SHA3_512>(rnd, skey, msg, ctx, computed_sig);
      } else if (hash_alg == "SHAKE-128") {
        sign_ok = ml_dsa_87::hash_sign<hash_algorithm::SHAKE_128>(rnd, skey, msg, ctx, computed_sig);
      } else if (hash_alg == "SHAKE-256") {
        sign_ok = ml_dsa_87::hash_sign<hash_algorithm::SHAKE_256>(rnd, skey, msg, ctx, computed_sig);
      } else {
...
// Use ML-DSA-87 HashML-DSA verify known answer tests from NIST ACVP Server (SHA3-family only).
TEST(ML_DSA, ML_DSA_87_HashVerify_ACVP_KnownAnswerTests)
{
  using hash_algorithm = ml_dsa_87::hash_algorithm;

  const std::string kat_file = "./kats/ml_dsa_87_hash_verify.acvp.kat";
  std::fstream file(kat_file);
  ...
  while (true) {
    ...
    if (!std::getline(file, msg_line).eof()) {
      ...
      if (hash_alg == "SHA3-224") {
        is_valid = ml_dsa_87::hash_verify<hash_algorithm::SHA3_224>(pkey, msg, ctx, sig);
      } else if (hash_alg == "SHA3-256") {
        is_valid = ml_dsa_87::hash_verify<hash_algorithm::SHA3_256>(pkey, msg, ctx, sig);
      } else if (hash_alg == "SHA3-384") {
        is_valid = ml_dsa_87::hash_verify<hash_algorithm::SHA3_384>(pkey, msg, ctx, sig);
      } else if (hash_alg == "SHA3-512") {
        is_valid = ml_dsa_87::hash_verify<hash_algorithm::SHA3_512>(pkey, msg, ctx, sig);
      } else if (hash_alg == "SHAKE-128") {
        is_valid = ml_dsa_87::hash_verify<hash_algorithm::SHAKE_128>(pkey, msg, ctx, sig);
      } else if (hash_alg == "SHAKE-256") {
        is_valid = ml_dsa_87::hash_verify<hash_algorithm::SHAKE_256>(pkey, msg, ctx, sig);
      } else {
        FAIL() << "Unknown hash algorithm: " << hash_alg;
      }
