...
// Use ML-DSA-44 sign_core (mu-taking, externalMu=True) known answer tests from NIST ACVP Server.
TEST(ML_DSA, ML_DSA_44_Sign_Core_ACVP_KnownAnswerTests)
{
  const std::string kat_file = "./kats/ml_dsa_44_sign_core.acvp.kat";
  std::fstream file(kat_file);
  ...
  while (true) {
    ...
    if (!std::getline(file, mu_line).eof()) {
      ...
      std::array<uint8_t, ml_dsa_44::SigByteLen> computed_sig{};
      const auto sign_ok = ml_dsa_44::sign_core(rnd, skey, mu, computed_sig);
      ASSERT_TRUE(sign_ok);
...
// Use ML-DSA-44 verify_core (mu-taking, externalMu=True) known answer tests from NIST ACVP Server.
TEST(ML_DSA, ML_DSA_44_Verify_Core_ACVP_KnownAnswerTests)
{
  const std::string kat_file = "./kats/ml_dsa_44_verify_core.acvp.kat";
  std::fstream file(kat_file);
  ...
  while (true) {
    ...
    if (!std::getline(file, mu_line).eof()) {
      ...
      const auto is_valid = ml_dsa_44::verify_core(pkey, mu, sig);
