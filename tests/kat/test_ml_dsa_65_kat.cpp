...
TEST(ML_DSA, ML_DSA_65_Sign_ACVP_KnownAnswerTests)
{
  ...
  while (true) {
    ...
    if (!std::getline(file, msg_line).eof()) {
      ...
      std::array<uint8_t, ml_dsa_65::SigByteLen> computed_sig{};
      const auto sign_ok = ml_dsa_65::sign(rnd, skey, msg, ctx, computed_sig);
      ASSERT_TRUE(sign_ok);
...
// Use ML-DSA-65 sign_core (mu-taking, externalMu=True) known answer tests from NIST ACVP Server.
TEST(ML_DSA, ML_DSA_65_Sign_Core_ACVP_KnownAnswerTests)
{
  const std::string kat_file = "./kats/ml_dsa_65_sign_core.acvp.kat";
  std::fstream file(kat_file);
  ...
  while (true) {
    ...
    if (!std::getline(file, mu_line).eof()) {
      ...
      std::array<uint8_t, ml_dsa_65::SigByteLen> computed_sig{};
      const auto sign_ok = ml_dsa_65::sign_core(rnd, skey, mu, computed_sig);
      ASSERT_TRUE(sign_ok);
...
// Use ML-DSA-65 verify_core (mu-taking, externalMu=True) known answer tests from NIST ACVP Server.
TEST(ML_DSA, ML_DSA_65_Verify_Core_ACVP_KnownAnswerTests)
{
  const std::string kat_file = "./kats/ml_dsa_65_verify_core.acvp.kat";
  std::fstream file(kat_file);
  ...
  while (true) {
    ...
    if (!std::getline(file, mu_line).eof()) {
      ...
      const auto is_valid = ml_dsa_65::verify_core(pkey, mu, sig);
