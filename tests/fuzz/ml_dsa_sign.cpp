...
LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
  ...
  constexpr size_t OFF_MALFORM_MU = OFF_MALFORM_SKEY + md::SecKeyByteLen;
  constexpr size_t OFF_MALFORM_END = OFF_MALFORM_MU + md::MessageRepresentativeByteLen;
  ...
  if (is_mode_a) {
    ...
    std::copy(data + OFF_MALFORM_SKEY, data + OFF_MALFORM_MU, skey.begin());
    std::copy(data + OFF_MALFORM_MU, data + OFF_MALFORM_END, mu.begin());
  } else {
