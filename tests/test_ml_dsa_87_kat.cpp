...
TEST(ML_DSA, ML_DSA_87_KnownAnswerTests)
{
  ...
  while (true) {
    ...
    if (!std::getline(file, seed0).eof()) {
      ...
      const auto seed = ml_dsa_test_helper::from_hex<ml_dsa_87::KeygenSeedByteLen>(seed2);
      ...
      const auto pkey = ml_dsa_test_helper::from_hex<ml_dsa_87::PubKeyByteLen>(pkey2); // Expected public key
      ...
      const auto skey = ml_dsa_test_helper::from_hex<ml_dsa_87::SecKeyByteLen>(skey2); // Expected secret key
      ...
      const auto rnd = ml_dsa_test_helper::from_hex<ml_dsa_87::SigningSeedByteLen>(rnd2); // Randomness for "randomized" message signing
      ...
      const auto sig = ml_dsa_test_helper::from_hex<ml_dsa_87::SigByteLen>(sig2); // Expected signature
      ...
      const auto has_signed = ml_dsa_87::sign(rnd, computed_skey, msg_span, ctx_span, computed_sig);
      const auto is_valid = ml_dsa_87::verify(computed_pkey, msg_span, ctx_span, computed_sig);
