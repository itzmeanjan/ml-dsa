...
{
  constexpr uint32_t α = ml_dsa_65::γ2 << 1;
  ...
  std::array<ml_dsa_field::zq_t, ml_dsa_65::l * ml_dsa_ntt::N> vec{};
  ...
  std::array<uint8_t, ml_dsa_65::ω + ml_dsa_65::l> encoded_hints{};
  ...
  ml_dsa_sampling::expand_mask<ml_dsa_65::γ1, ml_dsa_65::l>(seed, kappa, vec);
  ret_val ^= static_cast<uint8_t>(vec[0].raw() ^ vec[vec.size() - 1].raw());

  ml_dsa_polyvec::ntt<ml_dsa_65::l>(vec);
  ret_val ^= static_cast<uint8_t>(vec[0].raw() ^ vec[vec.size() - 1].raw());

  ml_dsa_polyvec::intt<ml_dsa_65::l>(vec);
  ret_val ^= static_cast<uint8_t>(vec[0].raw() ^ vec[vec.size() - 1].raw());

  ml_dsa_polyvec::highbits<ml_dsa_65::l, α>(vec, vec_high);
  ret_val ^= static_cast<uint8_t>(vec_high[0].raw() ^ vec_high[vec_high.size() - 1].raw());

  ml_dsa_polyvec::lowbits<ml_dsa_65::l, α>(vec, vec_low);
  ret_val ^= static_cast<uint8_t>(vec_low[0].raw() ^ vec_low[vec_low.size() - 1].raw());

  ml_dsa_polyvec::encode<ml_dsa_65::l, w1bw>(vec_high, encoded);
  ret_val ^= encoded[0] ^ encoded[encoded.size() - 1];

  ml_dsa_polyvec::decode<ml_dsa_65::l, w1bw>(encoded, decoded);
  ret_val ^= static_cast<uint8_t>(decoded[0].raw() ^ decoded[decoded.size() - 1].raw());

  const auto z_norm = ml_dsa_polyvec::infinity_norm<ml_dsa_65::l>(vec);
  ret_val ^= static_cast<uint8_t>(z_norm.raw());

  ml_dsa_polyvec::make_hint<ml_dsa_65::l, α>(vec, vec_high, vec_hint);
  ret_val ^= static_cast<uint8_t>(vec_high[0].raw() ^ vec_hint[vec_hint.size() - 1].raw());

  const auto count_1 = ml_dsa_polyvec::count_1s<ml_dsa_65::l>(vec_hint);
  ret_val ^= static_cast<uint8_t>(count_1);

  ml_dsa_bit_packing::encode_hint_bits<ml_dsa_65::l, ml_dsa_65::ω>(vec_hint, encoded_hints);
  ret_val ^= encoded_hints[0] ^ encoded_hints[encoded_hints.size() - 1];

  ml_dsa_bit_packing::decode_hint_bits<ml_dsa_65::l, ml_dsa_65::ω>(encoded_hints, decoded_hints);
...
main()
{
  if (test_ml_dsa_65() != DUDECT_NO_LEAKAGE_EVIDENCE_YET) {
    return EXIT_FAILURE;
  }
