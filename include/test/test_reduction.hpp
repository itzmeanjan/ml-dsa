...
test_decompose()
{
  ...
  for (size_t i = 0; i < rounds; i++) {
    ...
    const field::zq_t h = dilithium_utils::make_hint<alpha>(z_, r);
    const field::zq_t rz = dilithium_utils::use_hint<alpha>(h, r);

    const field::zq_t rz_ = dilithium_utils::highbits<alpha>(r + z_);
