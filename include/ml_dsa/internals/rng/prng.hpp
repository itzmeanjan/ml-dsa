...
namespace ml_dsa_prng {

// Pseudo Random Number Generator s.t. N (>0) -many random bytes are read from SHAKE256 Xof state, arbitrary many times
// s.t. SHAKE256 state is obtained by
//
...
//
// Note, std::random_device's behaviour is implementation defined feature, so this PRNG implementation doesn't guarantee
// that it'll generate cryptographic secure random bytes if you opt for using default constructor of this struct.
//
// I strongly suggest you read https://en.cppreference.com/w/cpp/numeric/random/random_device/random_device before using
// default constructor. When using explicit constructor, it's your responsibility to supply `bit_security_level / 8`
// -many random seed bytes.
